...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
void tarea_mensaje(void *p) {
	/*
	 char *s = ((datos_t*) p)->msg;
	 UART_HandleTypeDef uart = ((datos_t*) p)->huart;
	 SemaphoreHandle_t sem = ((datos_t*) p)->sem;

	 while (1) {
		 ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
		 xSemaphoreTake(sem, portMAX_DELAY);
		 HAL_UART_Transmit(&uart, (uint8_t*) s, (uint16_t) strlen(s), 100);
		 xSemaphoreGive(sem);
	 }
	 */
	uint32_t val;
	...
	strcpy(s, "BB\r\n");
	while (1) {
		xTaskNotifyWait(0, UINT32_MAX, &val, portMAX_DELAY);
		s[1] = (char) (val & 0x07) + 0x30;
...
void tarea_botones(void *p) {
	...
	while (1) {
		pines[0] = pulsador_activo(0);
		pines[1] = pulsador_activo(1);
		for (int i = 0; i < 2; i++) {
			debounce_fsm(&botones[i], pines[i]);
			if (botones[i].activo && !botones[i].anterior) {
				// xTaskNotifyGive(tareas[i]);
				xTaskNotify(tarea, i + 1, eSetValueWithOverwrite);
			}
...
int main(void) {
	...
	/* add threads, ... */
	/*
	parametros[0].huart = huart2;
	parametros[0].sem = mutex;
	parametros[1].huart = huart2;
	parametros[1].sem = mutex;
	strcpy(parametros[0].msg, "B1\r\n");
	strcpy(parametros[1].msg, "B2\r\n");
	for (int i = 0; i < 2; i++) {
		xTaskCreate(tarea_mensaje, "serie",
