...
// --- PIPLINE EN CORTEX-M4 ---
// Utilizado para convolución, análisis de datos, etc. Permite ejecutar varias instrucciones en paralelo para mejorar el rendimiento.
// - Fetch: El procesador lee la instrucción de la memoria.
// - Decode: El procesador interpreta la instrucción y determina qué operación realizar.
// - Execute: El procesador realiza la operación indicada por la instrucción.
// - Memory Access: Si la instrucción requiere acceso a memoria (como LDR o STR), el procesador realiza esa operación.
// - Write Back: El procesador escribe el resultado de la operación de vuelta a un registro o a la memoria.
// Lectura/escritura de registros especiales:
// MSR PSP, R0		/**/ Interpretation: Move to Special Register (Process Stack Pointer) 	/**/ Action: Establece el valor de R0 en el registro especial PSP
// MSR APSR, R0		/**/ Interpretation: Move to Special Register (Application Program Status Register) 	/**/ Action: Establece el valor de R0 en el registro especial APSR
// MRS R0, APSR		/**/ Interpretation: Move from Special Register (Application Program Status Register) 	/**/ Action: Carga el valor del registro especial APSR en R0
// CPSID i        /**/ Interpretation: Change Processor State (Disable Interrupts) 	/**/ Action: Deshabilita las interrupciones (modo privilegiado)
// CPSID f        /**/ Interpretation: Change Processor State (Disable FIQ) 		/**/ Action: Deshabilita las interrupciones FIQ (modo privilegiado)
// CPSIE i        /**/ Interpretation: Change Processor State (Enable Interrupts)
// CPSIE f        /**/ Interpretation: Change Processor State (Enable FIQ) 		/**/ Action: Habilita las interrupciones FIQ (modo privilegiado)

// Bloque IF-THEN (máximo 4 instrucciones):
// CMP R0, R1            /**/ Interpretation: Compare 						/**/ Action: Resta interna que no guarda el resultado, solo actualiza los flags para saltos condicionales
// ITTEE EQ                  /**/ Interpretation: If-Then-Then-Else (condición EQ) 	/**/ Action: Establece que las siguientes 4 instrucciones se ejecutarán solo si el flag Z está seteado (R0 == R1)
// ADDEQ R3, R4, R5      /**/ Interpretation: Add if Equal 					/**/ Action: R3 = R4 + R5 solo si Z=1
// ASREQ R3, R3, #1      /**/ Interpretation: Logical Shift Right if Equal 	/**/ Action: R3 = R3 >> 1 solo si Z=1
// ADDNE R3, R6, R7      /**/ Interpretation: Add if Not Equal 				/**/ Action: R3 = R6 + R7 solo si Z=0
// ASRNE R3, R3, #1      /**/ Interpretation: Logical Shift Right if Not Equal 	/**/ Action: R3 = R3 >> 1 solo si Z=0

// --- INSTRUCCIONES IT (If-Then) ---
// IT      /**/ Interpretation: Only one conditional instruction      /**/ Format: IT <cond>
// ITE     /**/ Interpretation: Two conditional instructions          /**/ Format: IT<x> <cond> (x = T or E)
// ITET    /**/ Interpretation: Three conditional instructions        /**/ Format: IT<x><y> <cond> (x,y = T or E)
// ITETT   /**/ Interpretation: Four conditional instructions         /**/ Format: IT<x><y><z> <cond> (x,y,z = T or E)
// --- EJEMPLOS DE USO ---
// IT EQ     /**/ Action: If Equal (Z set) -> Run next instr          /**/ Example: ADDEQ R0, R0, R1
// ITE GE    /**/ Action: If Greater/Equal (T) else Less Than (E)     /**/ Example: ADDGE R0, R0, R1 | ADDLT R0, R0, R3
// ITET GT   /**/ Action: If T (GT), E (LE), T (GT)                   /**/ Example: ADDGT R0, R1 | ADDLE R0, R3 | ADDGT R2, #1
// ITETT NE  /**/ Action: If T (NE), E (EQ), T (NE), T (NE)           /**/ Example: ADDNE R0, R1 | ADDEQ R0, R3 | ADDNE R2, #1 | MOVNE R5, R3
