...
Arduino_ILI9341 g_tft(GPIO_TFT_DC, GPIO_TFT_RST, GPIO_TFT_CS);
RREFont g_font;
...
void drawGradientMenu(bool clearBackground, uint8_t topOffset, uint16_t bottomOffset) {
  ...
  for (byte lineTFT = 0; lineTFT < HEIGHT_FLAME_EFFECT; lineTFT++) {
    ...
    g_tft.drawImageF(0, topOffset + lineTFT, SCR_WD, 1, gradientInstance);
    g_tft.drawImageF(0, bottomOffset - lineTFT - 1, SCR_WD, 1, gradientInstance);
  }
}
...
void drawGradientFault(const char *error) {
  ...
  gradientInstance[0] = gradientInstance[SCR_WD - 1] = FAULT_BACKGROUND_COLOR;
  g_tft.fillRect(0, 0, SCR_WD, SCR_HT, FAULT_BACKGROUND_COLOR);
  for (byte lineTFT = 0; lineTFT < HEIGHT_FLAME_EFFECT; lineTFT++) {
    for (byte slopeFactor = 0; slopeFactor < MIN_SLOPE_FACTOR_EFFECT; slopeFactor++) {
      for (byte idx = 0; idx < SCR_WD / 2; idx++) {
        gradientInstance[(SCR_WD / 2) - 1 + idx] = gradientInstance[(SCR_WD / 2) - 1 + idx + 1];
        gradientInstance[(SCR_WD / 2) - idx] = gradientInstance[(SCR_WD / 2) - idx - 1];
        if (idx != SCR_WD / 2 - 1) gradientInstance[(SCR_WD / 2) - idx - 1] = FAULT_BACKGROUND_COLOR;
      }
    }
    g_tft.drawImageF(0, lineTFT, SCR_WD, 1, gradientInstance);
    g_tft.drawImageF(0, SCR_HT - lineTFT - 1, SCR_WD, 1, gradientInstance);
  }
  configFont(g_font, &rreInstance_12x16, SYSTEM_FAULT_TEXT_COLOR, 1, 1, 1, DEFAULT_LETTER_SPACING_SIZE, 10, FAULT_BACKGROUND_COLOR, true);
  for (byte idx = 0; idx < strlen(error); idx++) {
    if (error[idx] == '\\' && (error[idx+1] == 'n' || error[idx+1] == 'r')) {
      ...
      errorInstance[idx] = '\0';
      g_font.printStr(ALIGN_CENTER, (SCR_HT - 40) / 2, errorInstance);
      g_font.printStr(ALIGN_CENTER, (SCR_HT + 6) / 2, (char*)&(error[strlen(errorInstance)+2]));
      return;
    }
  }
  g_font.printStr(ALIGN_CENTER, (SCR_HT - 16) / 2, (char*)error);
}
