...
SPISettings W25Q64xx_SPISettings(SPI_PERIPHERAL_SPEED, MSBFIRST, SPI_MODE0, DATA_SIZE_8BIT);
W25Q64xx g_flash(GPIO_W25Q64_CS_PIN);
...
W25Qxx_result_t W25Q64xx::begin(void) {
  pinMode(this->m_selectPin, OUTPUT);
  SSEL_IDLE(this->m_selectPin);
  spiTransaction(W25Q64xx_SPISettings, []() { });
  this->reset();
  delayMicroseconds(30); // The device will take approximately tRST=30us to reset
  this->m_deviceID = this->readID();
  return (this->m_deviceID == INCOMPATIBLE_MODULE ? W25QXX_INCOMPATIBLE_MODULE : W25QXX_OK);
}
